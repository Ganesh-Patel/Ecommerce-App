...
function AdminDashboard() {
    ...
    return (
        <div className="flex h-screen bg-gray-100">
            {/* Sidebar */}
            <div className={`fixed z-10 bg-teal-700 h-full w-64 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
                <div className="flex flex-col h-full text-white">
                    ...
                    <nav className="mt-10 flex-grow">
                        {['Orders', 'Users', 'Products', 'Sales', 'Settings'].map((item) => (
                            <Link
                                to={`/${item.toLowerCase()}`} // Define the path based on the item name (e.g., '/orders', '/users')
                                key={item}
